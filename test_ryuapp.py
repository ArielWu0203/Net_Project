    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        if body[0].table_id==2:
            for stat in sorted([flow for flow in body if flow.priority > 0],
                ...
                print("%08d %08d %16s %08d " %(ev.msg.datapath.id,stat.table_id,stat.match['ipv4_dst'],stat.packet_count))
        elif body[0].table_id==3:
            for stat in sorted([flow for flow in body if flow.priority > 0],
                               key=lambda flow: (flow.match['ipv4_src'])):
                print("%08d %08d %16s %08d " %(ev.msg.datapath.id,stat.table_id,stat.match['ipv4_src'],stat.packet_count))
